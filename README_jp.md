...
4. 結果を確認 — 参考範囲とスコア（同じ文の前回からの変化を含む）、誤りの位置を赤いマーカーで示す波形プレーヤー（Wav2Vec2-CTCタイムスタンプ、クリックでその位置を再生）、そして音節単位の字母比較・誤り一覧 / Whisper・Wav2Vec2・カタカナの比較 / LLMコーチングのタブ
...
*同じ発話をチャネルで見た画面 — 目標文、言語モデルが補正して聞き取ったWhisperの認識、スコア算出に使う補正なしの音響認識、そして学習者の発音をカタカナに戻した表記です。*
