...
5. 「学習記録」タブでスコアの推移と繰り返し出る誤り（弱点プロファイル）を確認
...
言語学コアは完全にユニットテストされています（122件）: 標準発音法に基づく表面形変換60件以上（形態音韻規則と境界誤検出の回帰ガードを含む）、IPAマッピング、アライメント演算、CTCタイムスタンプ伝播、統計ユーティリティ、すべてのL1エラータグを検証。
