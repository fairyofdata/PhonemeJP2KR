...
*화려한 도시를 그리며 찾아왔네 그 곳은 춥고도 험한 곳* を、日本語母語話者に典型的な読み方で日本語の声が読んだ例です。音声は管理者用のテキスト入力から、かなの台本（はりょはん どしるる ぐりみょ …）を TTS の Nanami に読ませて作りました。22音節の中に連音・鼻音化（찾아왔네 → [차자완네]）、濃音化（춥고도 → [춥꼬도]）、終声の中和（곳 → [곧]）が含まれます。
...
*同じ発話を3つのチャネルで見た画面 — 目標文、言語モデルが補正して聞き取ったWhisperの認識、スコア算出に使う音響認識（その下にカタカナ表記）を、語ごとにそろえています。Whisperは화려한・찾아왔네を文脈で復元しますが、도시르르・추부고도・홈한はそのまま聞いています。音響チャネルには日本語母語話者に典型的な誤りが並びます — ㅡ→ㅜ、ㅕ→ㅛ、パッチム後の母音挿入（도시루루、추부고도、호무한）、ㅓ→ㅗ。誤りのある8語のうち5語では、目標と実際の読みのカタカナが完全に同じです（トシルル、クリミョ、ク、コスン、ホムハン）。学習者に聞こえない差こそ、このシステムが測っているものです。*

**3つのチャネルと1つの表記。** 測定チャネルは目標（標準発音）・聞こえ方（Whisper）・実際の音（Wav2Vec2、スコアの根拠）の3つです。カタカナは4つ目のチャネルではなく、実際の音を固定の規則表（[`src/kana.py`](src/kana.py)）で日本語の音に置き換えた表記です。平音・激音・濃音、ㅓ/ㅗ、ㅡ/ㅜ、ㄴ/ㅇ パッチムが同じ文字になることを見せるために置いており、目標とWhisperには付けません。Wav2Vec2の出力には分かち書きがないため、採点と同じ字母アライメントを使って目標文の語に切り分けます（[`src/words.py`](src/words.py)）。語ごとの説明はタグごとの定型文で、LLMは使いません（[`docs/DECISIONS.md`](docs/DECISIONS.md) の決定11）。
...
言語学コアは完全にユニットテストされています（154件）: 標準発音法に基づく表面形変換60件以上（形態音韻規則と境界誤検出の回帰ガードを含む）、IPAマッピング、アライメント演算、CTCタイムスタンプ伝播、統計ユーティリティ、すべてのL1エラータグを検証。コア周辺も対象です: 実験6の結果から読み込む参考範囲、APIキーの解決順（環境変数 → `.env` → secrets）、録音の整理を含む学習記録の永続化、前後の雑音ルール（除外して**はいけない**ケースを含む）、管理者入力のCTC強制アライメント、分かち書きのない音響出力の語単位の整列とカタカナ表（統合すべき対立を含む）、そしてUIマークアップの音節グルーピングと学習者・ASR・LLMテキストのエスケープ。
