...
言語学コアは完全にユニットテストされています（82件）: 標準発音法に基づく表面形変換60件以上（形態音韻規則と境界誤検出の回帰ガードを含む）、IPAマッピング、アライメント演算、統計ユーティリティ、すべてのL1エラータグを検証。
...
今後の予定されている機能:
- **強制アライメント (Forced alignment)**（CTCセグメンテーション）: エラー発生箇所を時間軸で特定し、その部分だけを再生する機能
- **Kドラマシャドーイングモード**: 人気コンテンツから抽出したターゲット文のプリセット提供
- **日本語アクセントのL2韓国語音声でWav2Vec2をファインチューニング**: 第一候補は[AI-Hub 外国人韓国語発話音声データ](https://aihub.or.kr/aihubdata/data/view.do?currMenu=115&topMenu=100&aihubDataSe=realm&dataSetSn=505)（日本語母語話者を含む）— 実験2・4で文書化されたASRエラー交絡への対策
