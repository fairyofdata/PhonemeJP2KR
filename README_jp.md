...
| **解釈** | Gemini 2.5 Flash — 構造化された根拠（エラータグ・IPA・スコア）を入力 | 教育的役割のみ: コーチング文 |
...
日本語母語話者が *화려한 도시를 그리며 찾아왔네 그 곳은 춥고도 험한 곳* を読んだ例です。22音節の中に連音・鼻音化（찾아왔네 → [차자완네]）、濃音化（춥고도 → [춥꼬도]）、終声の中和（곳 → [곧]）が含まれます。
...
*同じ発話を3つのチャネルで見た画面 — 目標文、言語モデルが補正して聞き取ったWhisperの認識、スコア算出に使う補正なしの音響認識（その下にカタカナ表記）を、語ごとにそろえています。험한 → 홍한 には日本語母語話者に典型的な2つのタグ（ㅓ→ㅗ、ㅁ→ㅇ）が付きますが、カタカナ ホンハン では前者を表せません。*
...
言語学コアは完全にユニットテストされています（148件）: 標準発音法に基づく表面形変換60件以上（形態音韻規則と境界誤検出の回帰ガードを含む）、IPAマッピング、アライメント演算、CTCタイムスタンプ伝播、統計ユーティリティ、すべてのL1エラータグを検証。コア周辺も対象です: 実験6の結果から読み込む参考範囲、APIキーの解決順（環境変数 → `.env` → secrets）、録音の整理を含む学習記録の永続化、前後の雑音ルール（除外して**はいけない**ケースを含む）、分かち書きのない音響出力の語単位の整列とカタカナ表（統合すべき対立を含む）、そしてUIマークアップの音節グルーピングと学習者・ASR・LLMテキストのエスケープ。
...
- **トークンコストの最適化**: コストの高いマルチモーダル音声ストリーミングAPIを直接叩くのではなく、ローカル音響モデルで抽出した最小限の構造化証拠（`error_tags`、表面形IPA、スコア）のみをGemini 2.5 Flash（`temperature=0.2`）に渡すことで、リクエストあたりのプロンプトサイズを~350トークン以内（1回あたり$0.0001未満）に抑制しています。
