...
| **解釈** | Gemini 3.8 Flash — 構造化された根拠（エラータグ・IPA・スコア）を入力 | 教育的役割のみ: コーチング文 |
...
flowchart TD
    ...
    S --> L["Gemini 3.8 Flash<br/>構造化された根拠のみを解釈"]
...
言語学コアは完全にユニットテストされています（151件）: 標準発音法に基づく表面形変換60件以上（形態音韻規則と境界誤検出の回帰ガードを含む）、IPAマッピング、アライメント演算、CTCタイムスタンプ伝播、統計ユーティリティ、すべてのL1エラータグを検証。コア周辺も対象です: 実験6の結果から読み込む参考範囲、APIキーの解決順（環境変数 → `.env` → secrets）、録音の整理を含む学習記録の永続化、前後の雑音ルール（除外して**はいけない**ケースを含む）、管理者入力のCTC強制アライメント、分かち書きのない音響出力の語単位の整列とカタカナ表（統合すべき対立を含む）、そしてUIマークアップの音節グルーピングと学習者・ASR・LLMテキストのエスケープ。
...
- **トークンコストの最適化**: コストの高いマルチモーダル音声ストリーミングAPIを直接叩くのではなく、ローカル音響モデルで抽出した最小限の構造化証拠（`error_tags`、表面形IPA、スコア）のみをGemini 3.8 Flash（`temperature=0.2`）に渡すことで、リクエストあたりのプロンプトサイズを~350トークン以内に抑制しています。
