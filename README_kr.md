...
*화려한 도시를 그리며 찾아왔네 그 곳은 춥고도 험한 곳*을 일본어 화자의 전형적인 방식으로 일본어 목소리가 읽은 예입니다. 음성은 관리자용 텍스트 입력에서 가나 대본(はりょはん どしるる ぐりみょ …)을 TTS Nanami에게 읽혀 만들었습니다. 22음절 안에 연음·비음화(찾아왔네 → [차자완네]), 경음화(춥고도 → [춥꼬도]), 종성 중화(곳 → [곧])가 들어 있습니다.
...
*같은 발화를 세 채널로 본 화면입니다 — 목표 문장, 언어 모델이 보정해 들은 Whisper의 인식, 점수 계산에 쓰이는 음향 인식(그 아래 가타카나 표기)을 어절별로 맞췄습니다. Whisper는 화려한·찾아왔네를 문맥으로 복원하지만 도시르르·추부고도·홈한은 그대로 들었습니다. 음향 채널에는 일본어 화자의 전형적인 오류가 모입니다 — ㅡ→ㅜ, ㅕ→ㅛ, 받침 뒤 모음 삽입(도시루루, 추부고도, 호무한), ㅓ→ㅗ. 오류가 있는 8어절 중 5어절은 목표와 실제 읽기의 가타카나가 완전히 같습니다(トシルル, クリミョ, ク, コスン, ホムハン). 학습자에게 들리지 않는 그 차이가 바로 이 시스템이 재는 것입니다.*

**세 채널과 하나의 표기.** 측정 채널은 목표(표준 발음)·들린 것(Whisper)·실제 소리(Wav2Vec2, 점수의 근거) 세 개입니다. 가타카나는 네 번째 채널이 아니라 실제 소리 채널을 고정 규칙표([`src/kana.py`](src/kana.py))로 일본어 음운에 옮겨 적은 표기입니다. 평음/격음/경음, ㅓ/ㅗ, ㅡ/ㅜ, ㄴ/ㅇ 받침이 같은 글자로 뭉개진다는 점을 보여 주려고 두며, 목표와 Whisper에는 붙이지 않습니다. Wav2Vec2 출력에는 띄어쓰기가 없어서 채점과 같은 자모 정렬로 목표 문장의 어절에 잘라 맞춥니다([`src/words.py`](src/words.py)). 어절별 설명은 태그마다 정해진 문구이고 LLM을 쓰지 않습니다([`docs/DECISIONS.md`](docs/DECISIONS.md)의 결정 11).
...
언어학 코어는 완전히 단위 테스트되어 있습니다(154개): 표준발음법 기준 표면형 변환 60개 이상(형태음운 규칙과 경계 오탐 회귀 가드 포함), IPA 매핑, 정렬 연산, CTC 타임스탬프 전파, 통계 유틸리티, 모든 L1 오류 태그 검증. 코어 주변부도 함께 검증합니다: 실험 6 결과에서 읽어오는 참고 구간, API 키 탐색 순서(환경 변수 → `.env` → secrets), 녹음 정리를 포함한 학습 기록 저장, 앞뒤 잡음 규칙(제거하면 **안 되는** 경우 포함), 관리자 입력의 CTC 강제 정렬, 띄어쓰기 없는 음향 출력의 어절 정렬과 가타카나 규칙표(뭉개져야 하는 대립 포함), 그리고 UI 마크업의 음절 묶기와 학습자·ASR·LLM 텍스트 이스케이프.
