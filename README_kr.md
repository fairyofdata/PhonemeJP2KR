...
**어절 경계를 넘는 규칙.** 한 마디로 이어 읽으면 규칙이 경계도 넘습니다 — 밥 먹어 → [밤머거](§18 붙임), 옷 입어 → [온니버](§29 붙임2), 밭 아래 → [바다래](§15). 끊어 읽으면 [밥 머거]입니다. 둘 다 표준이므로 목표 발음을 두 가지로 만들고, **경계마다** 그 경계에서 오류가 적은 쪽을 채점에 씁니다(같으면 끊어 읽기). 선택 결과는 결과 객체와 내보내기에 남고, 어절 비교에서는 ‿로 표시합니다. 문장 단위가 아니라 경계마다 고르는 이유는 경계와 무관한 오류가 읽기를 결정하지 않게 하기 위해서입니다. 다루지 않는 것: 운율(실제로 어디서 끊었는지는 소리에서 추정할 뿐 측정하지 않습니다)과 §29의 어휘적 예외(곧이어 → [고디어]).
## 점수화와 L1 오류 분류 체계
