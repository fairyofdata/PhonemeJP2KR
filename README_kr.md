...
| **해석** | Gemini 3.8 Flash — 구조화된 근거(오류 태그, IPA, 점수)를 입력받음 | 교육적 역할만: 코칭 텍스트 |
...
flowchart TD
    ...
    S --> L["Gemini 3.8 Flash<br/>구조화된 근거만 해석"]
...
- **토큰 비용 최적화**: 고비용의 멀티모달 오디오 스트리밍 대신 로컬 음향 모델로 1차 정형화된 최소 진단 증거(`error_tags`, 표면형 IPA, 정렬 점수)만 Gemini 3.8 Flash(`temperature=0.2`)로 전달하여 요청당 프롬프트 크기를 ~350 토큰 이내로 극대화하여 비용을 통제합니다.
