...
| **해석** | Gemini 2.5 Flash — 구조화된 근거(오류 태그, IPA, 점수)를 입력받음 | 교육적 역할만: 코칭 텍스트 |
...
일본인 학습자가 *화려한 도시를 그리며 찾아왔네 그 곳은 춥고도 험한 곳*을 읽은 결과입니다. 22음절 안에 연음·비음화(찾아왔네 → [차자완네]), 경음화(춥고도 → [춥꼬도]), 종성 중화(곳 → [곧])가 들어 있습니다.
...
*같은 발화를 세 채널로 본 화면입니다 — 목표 문장, 언어 모델이 보정해 들은 Whisper의 인식, 점수 계산에 쓰이는 보정 없는 음향 인식(그 아래 가타카나 표기)을 어절별로 맞췄습니다. 험한 → 홍한에는 일본어 화자의 전형적인 태그 두 개(ㅓ→ㅗ, ㅁ→ㅇ)가 붙지만, 가타카나 ホンハン으로는 앞의 것을 나타낼 수 없습니다.*
...
언어학 코어는 완전히 단위 테스트되어 있습니다(148개): 표준발음법 기준 표면형 변환 60개 이상(형태음운 규칙과 경계 오탐 회귀 가드 포함), IPA 매핑, 정렬 연산, CTC 타임스탬프 전파, 통계 유틸리티, 모든 L1 오류 태그 검증. 코어 주변부도 함께 검증합니다: 실험 6 결과에서 읽어오는 참고 구간, API 키 탐색 순서(환경 변수 → `.env` → secrets), 녹음 정리를 포함한 학습 기록 저장, 앞뒤 잡음 규칙(제거하면 **안 되는** 경우 포함), 띄어쓰기 없는 음향 출력의 어절 정렬과 가타카나 규칙표(뭉개져야 하는 대립 포함), 그리고 UI 마크업의 음절 묶기와 학습자·ASR·LLM 텍스트 이스케이프.
